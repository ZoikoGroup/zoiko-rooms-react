...
export function Eyebrow({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <motion.span
      variants={fadeUp}
      className={`text-sm font-semibold uppercase tracking-[0.2em] text-brand-red ${className}`}
    >
      {children}
    </motion.span>
  );
...
export function SectionTitle({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <motion.h2
      variants={fadeUp}
      className={`max-w-2xl font-heading text-3xl font-medium text-brand-navy sm:text-4xl ${className}`}
    >
      {children}
    </motion.h2>
  );
