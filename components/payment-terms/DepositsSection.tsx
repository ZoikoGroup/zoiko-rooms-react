    ...
    <DocSection id="deposits">
      <Reveal className="flex flex-col gap-6">
        <div className="flex flex-col gap-1.5">
          <SectionHeading>{t("Deposits and rent in advance")}</SectionHeading>
          <Subtitle>{t('Different money types, kept distinct — never a generic "non-refundable deposit" switch.')}</Subtitle>
        </div>
              ...
              <div className="flex flex-col gap-1.5 border-t border-[#E9E0D3] pt-3">
                {facts.map((fact) => (
                  <div key={fact.label} className="flex items-center justify-between gap-3 text-sm">
                    <span className="text-neutral-500">{t(fact.label)}</span>
                    <span className="font-semibold text-brand-navy">{t(fact.value)}</span>
                  </div>
                ))}
              </div>
            </motion.div>
        ...
        <Paragraph>
          {t(
            "If a holding deposit is lawfully retained rather than refunded or applied, we show the reason code and a written explanation within the required period — never a silent deduction.",
          )}
        </Paragraph>

        <div className="flex flex-col gap-3">
          <SubHeading>{t("England checkout decision gate")}</SubHeading>
          <Paragraph>
            {t("Before any rent or deposit collection completes in England, the checkout runs through these checks automatically:")}
          </Paragraph>
          ...
          <motion.div variants={fadeUp} className="overflow-hidden rounded-2xl border border-[#E9E0D3]">
            <div className="grid grid-cols-[1.8fr_1fr] gap-4 bg-[#F1E7DA] px-5 py-3 text-xs font-semibold uppercase tracking-wide text-[#8A6A24]">
              ...
              <span>{t("If no or unresolved")}</span>
            </div>
            <div className="divide-y divide-[#E9E0D3] bg-[#FFFDF8]">
              {decisionGateRows.map((row) => (
                <div key={row.question} className="grid grid-cols-[1.8fr_1fr] gap-4 px-5 py-3.5 text-sm">
                  <span className="text-brand-navy">{t(row.question)}</span>
                  <span className="font-medium text-brand-red">{t(row.response)}</span>
                </div>
              ))}
            </div>
          </motion.div>
