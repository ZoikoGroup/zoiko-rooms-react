...
export function Eyebrow({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <motion.span
      variants={fadeUp}
      className={`text-xs font-semibold uppercase tracking-[0.15em] text-brand-red ${className}`}
    >
      {children}
    </motion.span>
  );
...
export function Subtitle({ children }: { children: React.ReactNode }) {
  return (
    <motion.p variants={fadeUp} className="max-w-3xl text-sm leading-relaxed text-neutral-500">
      {children}
    </motion.p>
  );
}
