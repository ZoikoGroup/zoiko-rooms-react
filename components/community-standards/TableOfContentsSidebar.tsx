        ...
        {tocItems.map((item, index) => {
          ...
          return (
            ...
            >
              {isActive && (
                <motion.span
                  layoutId="community-standards-toc-active"
                  transition={{ duration: 0.25, ease: easeOut }}
                  className="absolute inset-0 rounded-lg bg-brand-navy"
                />
              )}
              <span className={`relative z-10 ${isActive ? "text-white" : "text-neutral-500"}`}>
                {String(index + 1).padStart(2, "0")}
              </span>
              <span
                className={`relative z-10 transition-colors duration-200 ${
                  isActive ? "font-semibold text-white" : "text-neutral-500 hover:text-brand-navy"
                }`}
              >
                {t(item.label)}
              </span>
