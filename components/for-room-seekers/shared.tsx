...
export function DotSteps({ steps }: { steps: { title: string; description: string }[] }) {
  return (
    <div className="flex flex-col gap-6">
      {steps.map(({ title, description }) => (
        <motion.div key={title} variants={fadeUp} className="flex items-start gap-4">
          <span className="mt-1.5 h-2.5 w-2.5 shrink-0 rounded-full bg-brand-navy" />
          <div>
...
// used for the status and warning-sign breakdowns on this page.
