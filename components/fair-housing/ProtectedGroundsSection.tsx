      ...
      <Container>
        <Reveal className="flex flex-col gap-8">
          <div className="flex flex-col gap-4">
            <Eyebrow>{t("What We Prohibit")}</Eyebrow>
            <SectionTitle>{t("Protected grounds, platform-wide.")}</SectionTitle>
            <Paragraph>
              {t(
          ...
          <motion.div variants={fadeUp} className="overflow-x-auto">
            <div className="min-w-[720px] overflow-hidden rounded-2xl border border-[#E9E0D3]">
              <div className="grid grid-cols-[1.6fr_1fr_1.6fr] gap-4 bg-[#F1E7DA] px-6 py-3 text-xs font-semibold uppercase tracking-wide text-[#8A6A24]">
                <span>{t("Ground")}</span>
                <span>{t("Platform treatment")}</span>
                <span>{t("Notes")}</span>
              </div>
              <div className="divide-y divide-[#E9E0D3] bg-[#FFFDF8]">
                {protectedGrounds.map((row) => (
                  <div key={row.ground} className="grid grid-cols-[1.6fr_1fr_1.6fr] items-center gap-4 px-6 py-3.5 text-sm">
                    <span className="font-medium text-brand-navy">{t(row.ground)}</span>
                    <Pill label={t(row.treatment)} tone={row.tone} />
                    <span className="text-neutral-500">{t(row.notes)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex flex-col gap-4">
            <SubHeading>{t("Conduct we prohibit")}</SubHeading>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {prohibitedConduct.map(({ title, description }) => (
                <motion.div key={title} variants={fadeUp} className="flex flex-col gap-1.5 rounded-2xl border border-[#E9E0D3] bg-[#FFFDF8] p-5">
                  <h4 className="font-heading text-sm font-semibold text-brand-red">{t(title)}</h4>
