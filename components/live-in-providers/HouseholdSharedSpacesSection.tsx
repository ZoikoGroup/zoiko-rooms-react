      ...
      <Container>
        <Reveal className="flex flex-col gap-8">
          <div className="flex flex-col gap-4">
            <Eyebrow>{t("Household and Shared Spaces")}</Eyebrow>
            <SectionTitle>{t("Structured facts, not vague lifestyle labels")}</SectionTitle>
          </div>

          <motion.div variants={fadeUp} className="grid grid-cols-3 overflow-hidden rounded-2xl">
            {stripImages.map((image) => (
              <NaturalImage key={image.src} src={image.src} alt={t(image.alt)} />
            ))}
          </motion.div>

          <Callout label={t("Use specific facts")} className="max-w-3xl">
            {t(
              'Replace "looking for the right fit" with structured facts such as: quiet hours are 10:00 PM–7:00 AM; the bathroom is shared with one adult; cooking access is available until 10:00 PM; one cat lives in the home; overnight-guest rules are shown before application.',
            )}
          </Callout>
