      ...
      <Container>
        <Reveal className="flex flex-col gap-10">
          <div className="flex flex-col gap-4">
            ...
            <SectionTitle>{t("Living in the home does not automatically prove authority")}</SectionTitle>
            <Paragraph>
              {t(
                "Zoiko Rooms must separately confirm identity and the current legal or contractual authority to offer the room. The required evidence depends on ownership, tenancy, occupancy, and any applicable permission.",
              )}
            </Paragraph>
          </div>
          ...
          <motion.div variants={fadeUp} className="rounded-3xl border border-[#E9E0D3] bg-brand-cream/40 p-6 sm:p-8">
            <span className="text-xs font-semibold uppercase tracking-[0.15em] text-brand-red">
              {t("Authority is checked in stages")}
            </span>
            ...
            <p className="mt-5 text-sm leading-relaxed text-neutral-500">
              {t(
                "Resident awareness or an informal household agreement is not automatically the legal authority required to offer a room.",
              )}
            </p>
          </motion.div>
        </Reveal>
