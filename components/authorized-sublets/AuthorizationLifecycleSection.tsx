...
export function AuthorizationLifecycleSection() {
  const rows = mismatchRows.map((row) => [row.mismatch, row.response]);
      ...
      <Container>
        <Reveal className="flex flex-col gap-8">
          <div className="flex flex-col gap-4">
            <Eyebrow>Authorization Lifecycle</Eyebrow>
            <SectionTitle>Every stage is scoped and time-bound</SectionTitle>
          </div>
          ...
          <motion.div variants={fadeUp} className="rounded-3xl border border-[#E9E0D3] bg-white p-6 sm:p-8">
            ...
            <p className="mt-5 text-sm leading-relaxed text-neutral-500">
              Expiry, revocation, dispute, primary-agreement change, or provider move-out can pause
              or remove the listing immediately.
            </p>
          </motion.div>

          <div className="overflow-x-auto">
            <div className="min-w-[640px]">
              <InfoTable columns={["Mismatch", "Required response"]} rows={rows} />
            </div>
          </div>
        </Reveal>
      </Container>
